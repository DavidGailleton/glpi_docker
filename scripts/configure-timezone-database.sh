...
# Essayer d'abord avec la commande mariadb
if docker compose exec -i mariadb bash -c "command -v mariadb-tzinfo-to-sql >/dev/null 2>&1 && mariadb-tzinfo-to-sql /usr/share/zoneinfo | mariadb -uroot -p'${MYSQL_ROOT_PASSWORD}' mysql"; then
    echo -e "\e[32mDonnées de timezone chargées avec succès.\e[0m"
# Sinon essayer avec la commande mysql
elif docker compose exec -i mariadb bash -c "command -v mysql_tzinfo_to_sql >/dev/null 2>&1 && mysql_tzinfo_to_sql /usr/share/zoneinfo | mysql -uroot -p'${MYSQL_ROOT_PASSWORD}' mysql"; then
    echo -e "\e[32mDonnées de timezone chargées avec succès.\e[0m"
else
    ...
    # Méthode alternative si les commandes tzinfo échouent
    docker compose exec -i mariadb bash -c "mariadb -uroot -p'${MYSQL_ROOT_PASSWORD}' -e 'CALL mysql.mariadb_tzinfo_reload();' 2>/dev/null" || echo "Méthode alternative non disponible"
fi
...
if docker compose exec -i mariadb bash -c "
    mariadb -uroot -p'${MYSQL_ROOT_PASSWORD}' -e \"GRANT SELECT ON mysql.time_zone_name TO 'glpi'@'%';\" && \
...
TZ_COUNT=$(docker compose exec -i mariadb bash -c "mariadb -uroot -p'${MYSQL_ROOT_PASSWORD}' -s -N -e 'SELECT COUNT(*) FROM mysql.time_zone_name;'" 2>/dev/null)
...
docker compose exec -i mariadb bash -c "mariadb -uroot -p'${MYSQL_ROOT_PASSWORD}' -e \"SHOW GRANTS FOR 'glpi'@'%';\"" | grep -i time_zone || true
...
if docker compose exec -i php bash -c "mariadb -uglpi -p'${GLPI_PASSWORD}' -h mariadb -e 'SELECT COUNT(*) as \"Nombre de timezones accessibles\" FROM mysql.time_zone_name;' 2>&1"; then
